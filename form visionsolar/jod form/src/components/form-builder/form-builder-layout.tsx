...
export function FormBuilderLayout() {
  ...
  const [formTitle, setFormTitle] = useState('Vision Solar Assessment');
  ...
  const handleFinishBuilder = async () => {
    ...
    try {
      const formData = await formService.saveForm(formTitle, instances);
              ...
              <div className="text-center">
                {isEditingTitle ? (
                  <input
                    ...
                    className="text-xl font-bold text-charcoal leading-tight bg-off-white/50 border border-vision-green rounded px-2 outline-none text-center min-w-[200px]"
                  />
                ) : (
                  <h1 
                    onClick={() => setIsEditingTitle(true)}
                    className="text-xl font-bold text-charcoal leading-tight cursor-pointer hover:text-vision-green transition-colors border border-transparent hover:border-light-gray rounded px-2"
                  >
                    {formTitle}
                  </h1>
                )}
